...
the relative polar coordinates of all moving entities with respect to
the consumer:
\[s_t = \{r_1^t, \theta_1^t, r_2^t, \theta_2^t, ..., r_n^t, \theta_n^t, t\}\]

where \(r_i^t = \sqrt{(x_i^t - x_c^t)^2 + (y_i^t - y_c^t)^2}\) is the
distance from service \(i\) to consumer \(c\) at time \(t\), and
\(\theta_i^t = \arctan2(y_i^t - y_c^t, x_i^t - x_c^t)\) is the bearing
angle. The state is transformed into a feature vector containing
\([r, \cos\theta, \sin\theta]\) for each service, enabling the agent to
learn spatial relationships without explicit velocity computation.
...
replay memory. The agent begins with exploration-only (epsilon = 1.0) and
gradually shifts to exploitation as epsilon decays.
...
\begin{itemize}
	...
	\item
	      Lines 11-15: Agent invokes action based on epsilon value (random for
...
\[
	r_i = \sqrt{(x_i - x_c)^2 + (y_i - y_c)^2}, \quad
